...
1. **Service role:** `SUPABASE_SERVICE_KEY` and `supabaseAdmin` bypass RLS. Any new route using `supabaseAdmin` must not trust client-supplied IDs without **authorization** appropriate to the product (this repo has **no `middleware.ts`** at time of writing — treat missing auth on sensitive APIs as a **systemic finding**).
...
- [ ] **Security:** No secret leakage; service-role usage justified; risky inputs validated.
...
- [ ] **Performance:** Avoid N+1 Supabase/OpenAI calls; streaming backpressure/timeouts unchanged or improved.
