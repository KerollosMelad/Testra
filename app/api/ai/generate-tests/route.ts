...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const userStory = batchResult.value?.find((item: any) => item.id.toString() === workItemId);
      ...
      transformedUserStory = {
        ...
        assignedTo: userStory.fields['System.AssignedTo']?.displayName || undefined,
        priority: userStory.fields['Microsoft.VSTS.Common.Priority'] || undefined,
        acceptanceCriteria: userStory.fields['Microsoft.VSTS.Common.AcceptanceCriteria'] || undefined,
        tags: userStory.fields['System.Tags'] ? userStory.fields['System.Tags'].split(';').map((tag: string) => tag.trim()) : [],
      ...
      relatedTasks = batchResult.value?.filter((item: any) => {
        // Check if this item has a parent relationship to our work item
        if (item.relations) {
          return item.relations.some((relation: any) => 
            relation.rel.includes('Parent') && relation.url.includes(`workItems/${workItemId}`)
          );
        }
        return false;
      }).map((item: any) => ({
    ...
    const selectedModel = aiModel || project.aiModel || 'gpt-3.5-turbo';
    console.log('Selected AI model:', selectedModel);
    console.log('Project AI model:', project.aiModel);
